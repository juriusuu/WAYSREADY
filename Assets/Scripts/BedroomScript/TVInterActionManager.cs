...
public class TVInteractionManager : MonoBehaviour
{
    ...
    public TVNextPanelManager tvNextPanelManager; // Reference to the TVNextPanelManager script
    ...
    private System.Collections.IEnumerator HideWeatherUpdateUI()
    {
        ...
        // Notify the TVNextPanelManager to show the next panels
        if (tvNextPanelManager != null)
        {
            tvNextPanelManager.TriggerNextPanels();
        }

        // Stop and disable the TV audio source
        if (tvAudioSource != null)
        {
            tvAudioSource.Stop(); // Stop the audio
            tvAudioSource.enabled = false; // Disable the AudioSource
            Debug.Log("TV audio stopped and disabled.");
        }
    }
}
