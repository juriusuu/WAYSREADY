...
public class PostFireExtinguishersManager : MonoBehaviour
{
    ...
    public void ActivatePostInteraction()
    {
        if (isActivated)
        {
            Debug.LogWarning("Post-fire extinguishers interaction has already been activated.");
            return;
        }

        isActivated = true; // Mark as activated
        Debug.Log("Post-fire extinguishers interaction activated!");
    ...
    private System.Collections.IEnumerator ShowPanelsAndPlayAudio()
    {
        ...
        if (panel2 != null)
        {
            ...
            yield return new WaitForSeconds(4f); // Wait for 1.5 seconds
