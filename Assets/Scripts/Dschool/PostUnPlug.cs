...
public class PostUnplugAppliancesManager : MonoBehaviour
{
    ...
    public void ActivatePostInteraction()
    {
        if (isActivated)
        {
            Debug.LogWarning("Post-unplug appliances interaction has already been activated.");
            return;
        }

        isActivated = true; // Mark as activated
        Debug.Log("Post-unplug appliances interaction activated!");
