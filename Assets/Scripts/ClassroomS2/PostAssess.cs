...
public class PostAssessSurroundingsManager : MonoBehaviour
{
    ...
    public void ActivatePostInteraction()
    {
        if (isActivated)
        {
            Debug.LogWarning("Post-assess surroundings interaction has already been activated.");
            return;
        }

        isActivated = true; // Mark as activated
        Debug.Log("Post-assess surroundings interaction activated!");
